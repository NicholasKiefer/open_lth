...
class LRLotteryRunner(Runner):
    ...
    def __post_init__(self):
        self.desc.name_prefix = lambda: "lr_reset"
        self.desc.run_path = partial(self.desc.run_path, experiment="lr_reset")

    @staticmethod
    def description():
        return 'Run a lr reset lottery ticket hypothesis experiment.'
    ...
    def add_args(parser: argparse.ArgumentParser) -> None:
        ...
        LRLotteryRunner._add_levels_argument(lottery_parser)
        LotteryDesc.add_args(parser, defaults)

    @staticmethod
    def create_from_args(args: argparse.Namespace) -> 'LRLotteryRunner':
        return LRLotteryRunner(args.replicate, args.levels, LotteryDesc.create_from_args(args),
    ...
    def run(self) -> None:
        if self.verbose and get_platform().is_primary_process:
            print('='*82 + f'\nLottery Ticket LR Reset Experiment (Replicate {self.replicate})\n' + '-'*82)
    ...
    def _train_level(self, level: int):
        ...
        model = models.registry.load(self.desc.run_path(self.replicate, max(0, level - 1)), self.desc.train_end_step if level>0 else self.desc.train_start_step,
